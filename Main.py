...
.amsg{background:#f8fafc;border:1px solid #e2e8f0;border-radius:10px;padding:9px 11px;font-size:12px;line-height:1.6;color:#334155;margin-bottom:6px;}
.umsg{background:linear-gradient(135deg,#059669,#10b981);color:white;border-radius:10px;padding:9px 11px;font-size:12px;margin-bottom:6px;margin-left:10%;line-height:1.5;}
...
SECTOR_ICONS = {
    ...
    "Industrial":"⚙️","Industrial Con...":"⚙️","Retail":"🛒","Finance":"💰",
    "Life Sciences":"🔭","Consumer":"🛍️",
}
...
def build_table_rows(rows):
    ...
    for i, d in enumerate(rows):
        ...
        if risk >= 60:   risk_html, tr = f'<span class="risk-red">● {risk}</span>', "down"
        elif risk >= 40: risk_html, tr = f'<span class="risk-yel">● {risk}</span>', "neutral"
        else:            risk_html, tr = f'<span class="risk-grn">● {risk}</span>', "up"
        gs  = d["green_score"]
...
def show_all_alerts(hr2, lg2, data):
    ...
    if hr2:
        for d in hr2:
            st.markdown(f'<div class="alrt" style="border-left:3px solid #dc2626;"><div class="aico" style="background:#fef2f2;">⚠️</div><div><div class="at">{d["name"]}</div><div class="ad">{d["symbol"]} · Risk {d["financial_risk_score"]}/100 · {d["sector"]}</div></div></div>', unsafe_allow_html=True)
    else:
        st.info("Asnje kompani me risk te larte.")
    st.markdown("#### Low ESG Companies")
    if lg2:
        for d in lg2:
            st.markdown(f'<div class="alrt" style="border-left:3px solid #f59e0b;"><div class="aico" style="background:#fffbeb;">🏭</div><div><div class="at">{d["name"]}</div><div class="ad">{d["symbol"]} · Green {d["green_score"]}/100 · ESG {d["esg_rating"]}</div></div></div>', unsafe_allow_html=True)
    else:
    ...
    for d in sorted(data, key=lambda x: x["green_score"], reverse=True)[:5]:
        st.markdown(f'<div class="alrt" style="border-left:3px solid #3b82f6;"><div class="aico" style="background:#eff6ff;">🌿</div><div><div class="at">{d["name"]}</div><div class="ad">{d["symbol"]} · Green {d["green_score"]}/100 · ESG {d["esg_rating"]}</div></div></div>', unsafe_allow_html=True)
...
with h1:
    st.markdown('<div style="margin-bottom:10px;"><div style="font-size:18px;font-weight:700;color:#0f172a;">Welcome to Green Finance Intelligence! 👋</div><div style="font-size:11px;color:#64748b;margin-top:1px;">Here\'s what\'s happening with your green finance portfolio today.</div></div>', unsafe_allow_html=True)
with h2:
    st.markdown(f'<div style="display:flex;justify-content:flex-end;padding-top:3px;"><div class="pill"><div class="dot"></div>Live · {total} companies</div></div>', unsafe_allow_html=True)
...
with L:
    ...
    with hc1:
        st.markdown('<div style="display:flex;align-items:center;gap:5px;padding:4px 0;"><div class="dot"></div><span class="ct">Live Company Scores</span></div>', unsafe_allow_html=True)
    with hc2:
    ...
    st.markdown(f'<div style="background:white;border-radius:12px;border:1px solid #e2e8f0;overflow:hidden;"><table class="ctable"><thead><tr><th>Symbol</th><th>Company</th><th>Price</th><th>Change</th><th>Risk Score</th><th>Green Score</th><th>ESG</th><th>Sector</th><th>Trend</th></tr></thead><tbody>{rows_html}</tbody></table></div>', unsafe_allow_html=True)
...
# ══════════════════════════════════════════════════════════════════
st.markdown(
    '<div style="text-align:center;padding:6px;color:#94a3b8;font-size:9px;border-top:1px solid #f1f5f9;margin-top:6px;">'
    'Green Finance Intelligence · BigQuery · Vertex AI · Gemini · Real-Time S&amp;P 500 · '
    '<span style="color:#059669;font-weight:600;">Data Engineer</span></div>',
    unsafe_allow_html=True
)
